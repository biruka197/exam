...
async function sendAjaxRequest(action, bodyParams) {
  ...
  try {
    ...
    if (data.success) {
      if (action === "select_course") {
        const examListContainer = document.getElementById(
          "exam-list-container"
        );
        if (examListContainer) {
          if (mainContentArea) mainContentArea.style.display = "none";
          examListContainer.innerHTML = data.html;
          const animatedElement = examListContainer.querySelector(
            ".exam-list-animated"
          );
          if (animatedElement) {
            setTimeout(() => {
        ...
        if (
          contentContainer
            .querySelector("h2")
            ?.textContent.includes("Exam Complete!")
        ) {
          window.removeEventListener("beforeunload", confirmBeforeUnload);
        }
      }
      ...
      if (contentContainer)
        contentContainer.innerHTML = `<div class="m-4 p-4 text-red-800 bg-red-100 border border-red-300 rounded-lg">${
    ...
    console.error("Fetch failed:", error);
    if (contentContainer)
      contentContainer.innerHTML = `<div class="m-4 p-4 text-red-800 bg-red-100 border border-red-300 rounded-lg">An error occurred: ${error.message}</div>`;
  } finally {
...
async function submitAnswer(event) {
  ...
  const totalQuestions = parseInt(
    questionHeaderEl.textContent.split(" of ")[1]
  );
  ...
  if (currentQuestion === totalQuestions) {
    if (
      !confirm(
        "This is your last question. Are you sure you want to submit and finish the exam?"
      )
    ) {
      return;
    }
  ...
  try {
    ...
    alert(
      "Could not submit answer. Please check your connection and try again."
    );
...
function updateQuizUI(questionData, progressData) {
  ...
  document.getElementById(
    "progress-text"
  ).textContent = `${progressData.answered} of ${progressData.total} Answered`;
  ...
  for (const [key, optionText] of Object.entries(questionData.options)) {
    ...
    label.className =
      "flex items-center p-4 rounded-lg border border-slate-200 has-[:checked]:bg-green-50 has-[:checked]:border-green-500 cursor-pointer transition-all";
    label.innerHTML = `
...
function toggleAnswer() {
  ...
  fetch("index.php", {
    ...
    console.error("Failed to sync answer visibility state:", error)
  );
}
...
// --- Modern Exit Exam Confirmation ---

function showExitModal() {
    const modal = document.getElementById('exit-modal');
    const confirmBtn = document.getElementById('exit-modal-confirm-btn');
    const cancelBtn = document.getElementById('exit-modal-cancel-btn');

    modal.classList.remove('hidden');
    setTimeout(() => modal.classList.remove('opacity-0'), 10); // Fade in

    // Attach event listeners for the modal buttons
    confirmBtn.onclick = proceedWithExit;
    cancelBtn.onclick = hideExitModal;
}
...
function proceedWithExit() {
    ...
    confirmBtn.textContent = 'Exiting...';

    // Call the original AJAX request to exit the exam
    sendAjaxRequest("exit_exam", {});
}
...
function reportQuestion(questionNumber) {
  if (
    !confirm(
      "Are you sure you want to report this question as incorrect or problematic?"
    )
  ) {
    return;
  }
  ...
  fetch("index.php", {
    ...
    .then((data) => {
      if (data.success) {
        alert(
          "Thank you! The question has been reported to the administrator."
        );
        reportButton.textContent = "Reported!";
      } else {
        alert("Error: Could not report the question. " + (data.error || ""));
        reportButton.textContent = "Report Question";
        reportButton.disabled = false;
      }
    })
    .catch((error) => {
      console.error("Report submission failed:", error);
      alert("An error occurred while trying to report the question.");
      reportButton.textContent = "Report Question";
      reportButton.disabled = false;
    });
}
...
// The central AJAX function is the only one that needs a change.
// --- Modern Report Question Feature ---
...
function showReportModal(questionNumber) {
    ...
    // Store the question number on the button to retrieve it later
    ...
    cancelBtn.onclick = hideReportModal;
}
...
function handleReportConfirmation() {
    ...
    .then(data => {
        ...
        if (data.success) {
            if (data.status === 'already_reported') {
                showToast('This question has already been reported.');
            } else {
                showToast('Question reported successfully. Thank you!');
            }
            // Visually update the main report button on the page
            const mainReportBtn = document.getElementById('report-btn');
            mainReportBtn.textContent = '✓ Reported';
            mainReportBtn.disabled = true;
            mainReportBtn.classList.add('cursor-not-allowed', 'bg-slate-200');
            mainReportBtn.classList.remove('bg-slate-100');

        } else {
            showToast('Error: ' + (data.error || 'Could not report question.'), true);
        }
    })
    .catch(error => {
        hideReportModal();
    ...
    .finally(() => {
        // Re-enable the modal confirm button for future reports
