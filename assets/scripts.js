...
let timeLeft = 0, timerOn = true, timerInterval;

function updateTimer() {
  const timer = document.getElementById("timer");
  if (!timer) return;
  if (!timerOn) { timer.textContent = "--:--"; return; }
  if (timeLeft <= 0) {
...
function startTimer() {
  ...
  if (typeof timeLeft !== "number" || isNaN(timeLeft) || timeLeft < 0) timeLeft = 0;
...
async function sendAjaxRequest(action, bodyParams) {
  ...
  try {
    ...
    if (data.html) {
        ...
        if (contentContainer) contentContainer.innerHTML = `<div class="m-4 p-4 text-red-800 bg-red-100 border border-red-300 rounded-lg">${data.error}</div>`;
    }
    ...
    if(contentContainer) contentContainer.innerHTML = `<div class="m-4 p-4 text-red-800 bg-red-100 border border-red-300 rounded-lg">Request failed: ${error.message}</div>`;
  } finally {
...
function submitAnswer(event) {
    ...
    if (questionHeaderEl && progressTextEl) {
        ...
        const answeredCount = parseInt(progressParts[0], 10) + (selectedOption ? 1 : 0);
        if (currentQuestionNum === totalQuestions) {
            if (answeredCount < totalQuestions) {
                showCustomConfirm('You have unanswered questions. Are you sure you want to submit?', executeSubmit);
                proceed = false; 
            }
        }
    }
...
function selectCourse(courseName) { sendAjaxRequest("select_course", { course: courseName }); }
function proceedToExam(examCode) { sendAjaxRequest("proceed_to_exam", { exam_code: examCode }); }
function submitSettings(event) { event.preventDefault(); sendAjaxRequest("submit_settings", new URLSearchParams(new FormData(event.target))); }
function navigateToQuestion(index) { sendAjaxRequest("navigate_to_question", { navigate_to: index }); }
function toggleAnswer() { 
    const answerBox = document.querySelector(".answer-box");
    if(answerBox) answerBox.classList.toggle("show");
    fetch("index.php", { method: "POST", headers: { "X-Requested-With": "XMLHttpRequest" }, body: new URLSearchParams({ action: "toggle_answer" }) });
}
function exitExam() { showExitModal(); }
function retakeIncorrect() { sendAjaxRequest("retake_incorrect", {}); }
function restartQuiz() { sendAjaxRequest("restart_quiz", {}); }
...
function handleReportConfirmation() {
    ...
    fetch('index.php', {
        method: 'POST',
        headers: { 'X-Requested-With': 'XMLHttpRequest' },
        body: new URLSearchParams({ action: 'report_question', question_number: questionNumber })
    }).then(res => res.json()).then(data => {
        hideReportModal();
        if(data.success) {
            showToast(data.status === 'already_reported' ? 'This question has already been reported.' : 'Question reported. Thank you!');
