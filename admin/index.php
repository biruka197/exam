...
if ($page === 'admin' && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_exam'])) {
    ...
    if (empty($course)) {
        ...
        for ($i = 0; $i < count($files['name']); $i++) {
            ...
            $exam_path = 'admin/exams/' . $filename;
...
if ($page === 'result') {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (isset($_POST['restart'])) {
            unset($_SESSION['questions'], $_SESSION['answers'], $_SESSION['subject'], $_SESSION['num_questions'], $_SESSION['current_question'], $_SESSION['show_answer'], $_SESSION['timer_on'], $_SESSION['start_time'], $_SESSION['timer_duration']);
            header("Location: index.php?page=course_selection");
            exit;
        }
        if (isset($_POST['retake'])) {
            if (!empty($incorrect_questions)) {
                ...
                $_SESSION['show_answer'] = [];
                $_SESSION['start_time'] = time();
                $_SESSION['timer_duration'] = count($incorrect_questions) * 60;
                $_SESSION['timer_on'] = true;
                header("Location: index.php?page=quiz");
                exit;
            } else {
                $errors[] = "No incorrect questions to retake.";
            }
        }
    }
}
    ...
    <style>
        * {
        ...
        .form-group input:focus, .form-group select:focus {
            border-color: #006633;
            box-shadow: 0 0 8px rgba(0, 102, 51, 0.2);
            outline: none;
        }
        .total-questions {
            font-size: 12px;
        ...
        <?php elseif ($page === 'admin'): ?>
            ...
            <form method="POST" enctype="multipart/form-data" class="admin-form">
                ...
                <div class="drop-zone" id="dropZone">
                    ...
                    <p>Drag & Drop JSON Files Here or Click to Browse</p>
                    <input type="file" name="exam_files[]" id="exam_files" accept=".json" multiple required>
                </div>
                <ul class="file-list" id="fileList"></ul>
                <button type="submit" name="upload_exam"><span>⬆️</span> Upload Exams</button>
            </form>
            ...
            <table class="exam-table">
                ...
                <tbody>
                    <?php
                    $result = $conn->query("SELECT course, exam, course_id, exam_code FROM course");
                    while ($row = $result->fetch_assoc()):
                    ?>
                        <tr>
                            <td><?php echo htmlspecialchars($row['course']); ?></td>
                            <td><?php echo htmlspecialchars(basename($row['exam'])); ?></td>
            ...
            <script>
                ...
                function handleFiles(files) {
                    const validFiles = Array.from(files).filter(file => {
                        ...
                        if (!isJson) {
                            showError(`File "${file.name}" is not a JSON file. Only JSON files are allowed.`);
                        }
                        return isJson;
                    });
        ...
        <?php elseif ($page === 'quiz'): ?>
            ...
            <script>
                ...
                function navigateTo(index) {
                    ...
                    if (selectedOption) {
                        document.forms[0].elements['option'].value = selectedOption.value;
                    }
